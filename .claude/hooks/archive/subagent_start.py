...
def main():
    try:
        ...
        # NEW: Send SubagentStart event to events stream for frontend agent detection
        if not args.no_server:
            try:
                # Calculate depth for event (safe default if no parent)
                event_depth = calculate_session_depth(parent_session_id) if parent_session_id else 0

                subagent_start_event = create_hook_event(
                    ...
                    payload={
                        ...
                        "tools_granted": agent_info.get('tools_granted', []),
                        "parent_session_id": parent_session_id,
                        "depth_level": event_depth,
                        "metadata": {
                            "agent_type": agent_type,
                ...
                if send_event_to_server(subagent_start_event):
                    print(f"SubagentStart event created for {agent_info['agent_name']}", file=sys.stderr)
            except Exception as e:
                print(f"Failed to create SubagentStart event: {e}", file=sys.stderr)
